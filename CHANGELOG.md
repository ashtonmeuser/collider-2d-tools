...
- Add option for `SvgCollider2D` to merge generated colliders into a target `CompositeCollider2D` when available. If enabled, properties of the `CompositeCollider2D` are copied to uncomposable `EdgeCollider2D`.
